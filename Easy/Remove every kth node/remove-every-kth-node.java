...
class Solution
{
    ...
    {
        // If k is 1, return null (no nodes will remain)
        if (k == 1)
            return null;
        ...
        while (current != null) {
            count++;
            if (count % k == 0) { // Check if the current node is kth node
                if (previous == null) { // If previous is null, update head to skip the kth node
                    head = current.next;
                } else {
