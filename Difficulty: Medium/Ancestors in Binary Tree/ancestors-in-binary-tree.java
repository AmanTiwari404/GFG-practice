...
class Solution {
    ...
    public ArrayList<Integer> Ancestors(Node root, int target) {
        ...
        return new ArrayList<>();  // Return an empty list if target is not found
    }
    ...
    private boolean findAncestors(Node node, int target, ArrayList<Integer> ancestors) {
        ...
        if (findAncestors(node.left, target, ancestors) || findAncestors(node.right, target, ancestors)) {
            // If the target node is found in either subtree, add the current node to the ancestors list
